...
class NetEngine:
            ...
            for stun_host, stun_port in STUN_SERVERS:
                ...
                try:
                    _nat_type, gamehost, gameport = stun.get_ip_info(
                        "0.0.0.0", local_port, stun_host=stun_host, stun_port=stun_port
                    )
                    if gameport is None:
    ...
    def setup_addr_name(self):
        ...
        try:
            ...
            self.game.add_message(
                "Match server rejected registration: HTTP %d" % err.code
            )
            return False
    ...
    def connect(self, address):
        connect_thread = threading.Thread(
            target=self.connect_thread_go, args=(address,), daemon=True
        )
    ...
    def connect_thread_go(self, addr):
        ...
        url = MATCH_SERVER + "/connect2/chesschase0/%s/%s/" % (
            quote_path(self.address),
            quote_path(addr.lower()),
        )
    ...
    def add_peers_json(self, peers_json):
        for addresses in json.loads(peers_json):
            ...
            if self.address:
                self.game.add_message(
                    "Your address is still: %s" % self.address.upper()
                )
    ...
    def communicate(self):
        ...
        packet = marshal.dumps(
            (
                ...
                [
                    (
                        i,
                        self.iter_actions.setdefault(i, {}).setdefault(
                            self.game.my_id, []
                        ),
                    )
                    for i in range(
        ...
        if time_since_comm >= self.comm_gap_msg_at:
            if self.had_peer_packet:
                self.game.add_message(
                    "No communication for %d seconds" % self.comm_gap_msg_at
                )
            else:
                self.game.add_message(
                    "Waiting for direct UDP packets for %d seconds"
                    % self.comm_gap_msg_at
                )
        ...
        if self.game.mode == "replay":
            ...
            if any_actions(all_actions):
                ...
                if self.replay_wait == self.replay_max_wait:
                    self.replay_wait = 0
                    while (
                        not any_actions(all_actions)
                        and self.game.counter + 1 < self.replay_stop
                    ):
                        self.game.counter += 1
                        all_actions = self.get_replay_actions()
        ...
        for i, actions in all_actions:
            for action_type, params in actions:
                ...
                if action_func is None:
                    ...
                    if prev_messages == len(self.game.messages) and not getattr(
                        action_func, "quiet", False
                    ):
                        self.game.add_message(
                            "%s did %s" % (self.game.nick(i), action_type.upper())
                        )
    ...
    def iteration(self):
        ...
        if (
            self.game.mode != "replay"
            and self.game.my_id
            not in self.iter_actions.setdefault(self.game.counter + self.latency, {})
        ):
            self.iter_actions[self.game.counter + self.latency][self.game.my_id] = (
