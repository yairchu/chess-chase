...
class BoardView(Widget):
    ...
    def resized(self, *args):
        ...
        self.board_x = (
            self.x + (self.width - self.square_size * self.game.board_size[0]) / 2
        )
        self.board_y = self.top - self.square_size * (
            self.game.board_size[1] + self.top_margin_squares
        )
            ...
            for (x, y), col in cols.items():
                ...
                if (x, y) in self.game.board:
                    ...
                    if self.game.board[x, y].freeze_until > self.game.counter:
                        freeze_ratio = (
                            piece.freeze_until - self.game.counter
                        ) / piece.freeze_time
                        Color(0.90, 0.73, 0.38, 0.72)
                        Rectangle(
                            pos=(sx, sy),
                            size=(
                                (self.square_size - 1) * freeze_ratio,
                                self.square_size - 1,
                            ),
                        )

            for pos, piece in sorted(
                self.game.board.items(), key=lambda item: -self.screen_pos(item[0])[1]
            ):
                if pos not in see:
                    continue
                transparent = False
                        ...
                        if piece.last_pos is not None:
                            ...
                            Rectangle(
                                ...
                                pos=[
                                    int(
                                        last_pos[i]
                                        + (new_pos[i] - last_pos[i]) * pos_between
                                    )
                                    for i in range(2)
                                ],
                                size=piece_size,
                ...
                if piece is self.selected and self.game.active():
                    transparent = True
                Color(1, 1, 1, 0.5 if transparent else 1)
                self.draw_piece(piece, pos)

            if (
                self.selected is not None
                and self.dst_pos is not None
    ...
    def board_info(self):
        ...
        for piece in list(self.game.board.values()):
            ...
            if self.can_control(piece):
                ...
                if (
                    self.mouse_pos in moves
                    and not self.is_dragging
                    and piece == self.selected
                ):
            ...
            for dst in itertools.chain(piece.sight()):
                ...
                if dst in moves:
                    movesee[dst] = list(
                        map(operator.add, movesee.get(dst, [0] * 3), piece.sight_color)
                    )
        ...
        for pos, col in movesee.items():
            cols[pos] = [118 + a * 92.0 / max(col) for a in col]
    ...
    def on_touch_down(self, event):
        ...
        if self.is_choice_event(event):
            ...
            self.selected = self.potential_pieces[
                (self.potential_pieces.index(self.selected) + d)
                % len(self.potential_pieces)
            ]
            return
        if self.mouse_pos in self.game.board and self.can_control(
            self.game.board[self.mouse_pos]
        ):
    ...
    def calc_mouse_pos(self, pos):
        board_pos = [
            int((x - sx) // self.square_size)
            for x, sx in zip(pos, (self.board_x, self.board_y))
        ]
    ...
    def update_dst(self):
        if (
            self.selected is not None
            and self.game.board.get(self.selected.pos) is not self.selected
        ):
            self.selected = None
        ...
        if [] == self.potential_pieces:
            ...
            if (
                ...
                or self.selected not in self.potential_pieces
            ):
