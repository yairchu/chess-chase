...
def parse_args(argv):
    ...
    parser.add_argument(
        "--dev-state",
        choices=["menu", "setup", "tutorial", "play", "demo"],
        default="menu",
    )
    parser.add_argument(
        "--screenshot", help="write a PNG after opening the requested dev state"
    )
    parser.add_argument(
        "--window-size", default="1200x800", help="WIDTHxHEIGHT for dev screenshots"
    )
    parser.add_argument(
        "--exit-after",
        type=float,
        default=0.5,
        help="seconds to wait before screenshot/exit",
    )
...
TEXT_COLOR = (0.92, 0.94, 0.92, 1)
MUTED_TEXT_COLOR = (0.72, 0.76, 0.74, 1)
BUTTON_COLOR = (0.20, 0.22, 0.22, 1)
PRIMARY_BUTTON_COLOR = (0.16, 0.39, 0.36, 1)
GOLD_COLOR = (0.86, 0.64, 0.32, 1)
PANEL_COLOR = (0.03, 0.05, 0.05, 0.82)
...
class RecentMovesView(Widget):
    ...
    def show(self):
        ...
        with self.canvas:
            ...
            Rectangle(
                texture=label.texture, pos=(x, y + icon + 6), size=label.texture.size
            )
            if not pieces:
                return
            ...
            for piece in pieces:
                remaining = (
                    max(
                        piece.freeze_until,
                        self.game.player_last_move.get(piece.player, 0)
                        + self.game.player_freeze_time,
                    )
                    - self.game.counter
                )
...
class LogoView(Widget):
    ...
    def show(self):
        ...
        scale = min(self.width * 0.9 / tw, self.height * 0.95 / th)
...
class ConnectingView(Widget):
    ...
    def show(self):
        ...
        with self.canvas:
            ...
            for i in range(3):
                Color(0.22, 0.57, 0.86, 0.35 + 0.2 * i)
...
class Game(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        with self.info_pane.canvas.before:
            ...
            self.info_pane_bg = Rectangle(
                pos=self.info_pane.pos, size=self.info_pane.size
            )
        ...
        self.menu_top_spacer = Widget(size_hint=(1, 0.55))
        ...
        self.button_pane = BoxLayout(
            orientation="vertical", size_hint=(1, 0.4), spacing=12
        )
        ...
        self.button_pane.add_widget(
            style_button(
                WrappedButton(
                    ...
                    text="Start Game"
                    if env.is_mobile
                    else "Start Game: Play with friends",
                    on_press=self.start_game,
                ),
                primary=True,
            )
        )
        self.menu_button = style_button(
            Button(
                halign="center",
                size_hint=(1, 0),
                size_hint_min_y=38,
        ...
        self.menu_button.background_color = (0.06, 0.08, 0.08, 0.65)
        self.menu_button.font_size = "18sp"

        self.score_label = Label(
            halign="center", color=MUTED_TEXT_COLOR, font_size="21sp", **row_args
        )
        ...
        self.tutorial_label.bind(
            size=lambda *_: setattr(
                self.tutorial_label, "text_size", self.tutorial_label.size
            )
        )
        self.message_view = ScrollView(size_hint=(1, 1), do_scroll_x=False, bar_width=0)
        ...
        self.text_input = TextInput(
            ...
            background_normal="",
            background_active="",
            background_color=(0.04, 0.07, 0.07, 0.96),
    ...
    def refresh_layout(self):
        ...
        if screen == "game":
            self.orientation = (
                "horizontal" if self.size[0] > self.size[1] else "vertical"
            )
    ...
    def update_label(self):
        ...
        if self.game_model.mode == "tutorial":
            self.label.text = (
                self.game_model.messages[-1] if self.game_model.messages else ""
            )
    ...
    def resized(self, *args):
        if self.screen() != "game":
            ...
            if self.screen() == "menu":
                ...
                self.button_pane.size_hint = (1, 0.35)
        ...
        if self.orientation == "horizontal":
            ...
            self.button_pane.orientation = "vertical"
            self.button_pane.size_hint = (1, 0.4)
            self.button_pane.size_hint_min_y = 140
        else:
            self.info_pane.size_hint = (
                1,
                0.18 if self.game_model.mode == "tutorial" else p,
            )
    ...
    def king_captured(self, who):
        ...
        self.game_model.add_message("")
        self.game_model.add_message(
            "%s King Captured!" % self.game_model.player_str(who)
        )
...
class ChessChaseApp(App):
    ...
    def build(self):
        ...
        if self.dev_args and (
            self.dev_args.screenshot or self.dev_args.dev_state != "menu"
        ):
        ...
        for player in range(2):
            pieces.extend(
                sorted(
                    (
                        piece
                        for piece in self.game.game_model.board.values()
                        if piece.player == player
                    ),
                    key=lambda piece: piece.pos,
                )[:3]
            )
...
if __name__ == "__main__":
    ...
    Window.clearcolor = (0.04, 0.05, 0.05, 1)
    if args.screenshot:
